        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.order', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) { // nhận kết quả trả về
                        console.log(data);
                        var start_time = moment().format('YYYY-MM-DD') + ' ' + data.start_time;
                        var start_at = moment(start_time, 'YYYY-MM-DD HH:mm:ss').format('HH:mm');
                        var date_d_m = moment(data.date).format('DD/MM/YYYY');
                        show_time_price = data.price;
                        $('#theater_name').text(data.theater_name);
                        $('#theater_address').text(data.theater_address);
                        $('#time_start').text(start_at);
                        $('#showtime_date').text(date_d_m);
                        $('#movie_name').text('{{$movie->name}}');
                        $('#room_theater').text(' phòng chiếu:' + data.room_name);
                        $('#showtimeID').val(data.id_showtime);
                        //in ra sơ đồ ghế
                        $('#row_showtime').html('');
                        $('#column_showtime').html('');
                        $.each(data.room_seets, function(row, value) {
                            $('#row_showtime').append(`<tr>
                            <td>${row}</td>
                        </tr>`);
                            $('#column_showtime').append(`<tr id='row-${row}'></tr>`);
                            $.each(value, function(index, column) {
                                if (data.booked_ticket[row]) {
                                    if (data.booked_ticket[row].includes(column)) {
                                        $(`#row-${row}`).append(`<td><input type='checkbox' class='btn-check seat_check'  id='${row}${column}' name='seats[${row}][]' value='${column}' disabled><label class='class="btn btn-secondary p-0"' for='${row}${column}'>${row}${column}</label></td>`)
                                    } else {
                                        $(`#row-${row}`).append(`<td><input type='checkbox' class='btn-check seat_check'  id='${row}${column}' name='seats[${row}][]' value='${column}'><label class='btn btn-outline-danger p-0' for='${row}${column}'>${row}${column}</label></td>`)
                                    }
                                } else {
                                    $(`#row-${row}`).append(`<td><input type='checkbox' class='btn-check seat_check'  id='${row}${column}' name='seats[${row}][]' value='${column}'><label class='btn btn-outline-danger p-0' for='${row}${column}'>${row}${column}</label></td>`)
                                }
            ...
            // xử lý tổng giá tiền:
            var total = 0; // Biến để lưu tổng giá trị
            $(document).on('change', '.seat_check', function() {
                var seat_code = $(this).attr('id');
                var isChecked = $(this).is(":checked");
                if (isChecked) {
                    total += show_time_price;
                    $('#seat_number').append(`<span class="text-danger ${seat_code}">${seat_code} </span>`);
                } else {
                    total -= show_time_price;
                    $('#seat_number').find(`.${seat_code}`).remove();
                }
                $('#total_price').text(total.toLocaleString('vi-VN') + ' đ');
            });
            // checklogin
